...
namespace UI
{
	...
	UIManager::~UIManager()
	{
		...
		std::cout << "UnInit UI" << std::endl;
	}

	void UIManager::enableDocking(bool _value)
	{
		mDockingState = _value;
		if (_value)
	...
	void UIManager::applyStyle(EStyle _style)
	{
		...
		else if (_style == EStyle::ALTERNATIVE_DARK)
		{
			style->WindowPadding = ImVec2(15, 15);
	...
	bool UIManager::loadFont(const std::string& id, const std::string& path, float fontSize)
	{
		if (mFonts.find(id) == mFonts.end())
		{
			auto& io = ImGui::GetIO();
			ImFont* fontInstance = io.Fonts->AddFontFromFileTTF(path.c_str(), fontSize);

			if (fontInstance)
			{
				mFonts[id] = fontInstance;
				return true;
			}
		}
